...
import { startCursorTelegramWatcher } from './telegramWatcher.js';
...
describe('startCursorTelegramWatcher', () => {
  ...
  it('posts a question that appears after startup', async () => {
    ...
    expect(posts[0]).toContain('Cursor question');
    expect(posts[0]).toContain('<code>my-project</code>');
    expect(posts[0]).toContain('Q: Ship it? (options: Yes, No)');
    expect(posts[0]).toContain('Reply to this message to answer.');
  });
  ...
  it('stops polling once stopped', async () => {
    ...
    const watcher = startCursorTelegramWatcher({
      repo: { createIfNew: async () => null, setMessageId: async () => {} } as never,
      botToken: 't',
      chatId: 'chat1',
      intervalMs: 5,
      findPending,
